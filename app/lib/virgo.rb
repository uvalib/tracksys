...
# external source, namely the U.Va. Library catalog
  ...
  def self.get_metadata_json(marc_string, compare_barcode)
   ...
   # Publish a unit to Virgo (the DL)
   def self.publish(unit, logger = Logger.new(STDOUT))
      logger.info "Publish unit #{unit.id} to the DL"
      if unit.metadata.availability_policy.nil?
         raise "Metadata #{unit.metadata.id} for Unit #{unit.id} has no availability value.  Please fill in and retry."
      end
