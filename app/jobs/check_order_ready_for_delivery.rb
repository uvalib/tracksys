class CheckOrderReadyForDelivery < BaseJob
   ...
   def do_workflow(message)
      ...
      resp = Job.submit("/orders/#{message[:order_id]}/pdf", nil) == false
      if !resp.success?
         log_failure("Unable to generate PDF: #{err}")
      end
      ...
      resp = Job.submit("/orders/#{message[:order_id]}/email", nil) == false
      if !resp.success?
         log_failure("Unable to create order email: #{err}")
      end
   end
