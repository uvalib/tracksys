class CreateDlDeliverables < BaseJob
  ...
  def perform(message)
    Job_Log.debug "CreateDlDeliverablesProcessor received: #{message.to_json}"
    ...
    if @object.filename.match(".jp2$")
      ...
      IngestJp2k.exec_now( { :object_class => @object_class , :object_id => @object_id,  :jp2k_path => jp2k_path, :last => @last, :source => @source } )
