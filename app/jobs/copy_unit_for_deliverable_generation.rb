   ...
   def do_workflow(message)
      ...
      if failure_messages.empty?
         if message[:mode] == 'patron'
            on_success "Unit #{unit.id} has been successfully copied to #{destination_dir} so patron deliverables can be made."
            QueueUnitDeliverables.exec_now({ :unit => unit, :source => destination_dir }, self)
         elsif message[:mode] == 'dl'
            on_success "Unit #{unit.id} has been successfully copied to #{destination_dir} so Digital Library deliverables can be made."
            UpdateUnitDateQueuedForIngest.exec_now({ :unit => unit, :source => destination_dir }, self)
         elsif message[:mode] == 'both'
            on_success "Unit #{unit.id} has been successfully copied to both the DL and patron process directories"
            QueueUnitDeliverables.exec_now({ :unit => unit, :source => destination_dir }, self)
            UpdateUnitDateQueuedForIngest.exec_now({ :unit => unit, :source => destination_dir }, self)
         else
