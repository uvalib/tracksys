ActiveAdmin.register MasterFile do
   ...
   sidebar "Thumbnail", :only => [:show],  if: proc{ !master_file.deaccessioned? } do
      div :style=>"text-align:center" do
         if !master_file.metadata.nil?
            ...
            md_pid = nil
            md_pid = master_file.metadata.pid
            image_tag(
   ...
   member_action :download_from_archive, :method => :get do
      ...
      resp = Job.submit("/units/#{mf.unit_id}/copy",{computeID: current_user.computing_id, filename: mf.filename})
      if resp.success?
         ...
         redirect_to "/admin/units/#{mf.unit_id}", :alert => "Unable to download file: #{resp.message}"
      end
   end
