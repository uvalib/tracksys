class Customer < ApplicationRecord
   ...
   scope :has_unpaid_invoices, ->{
      ...
      .where("orders.order_status <> ?", "canceled").where('invoices.date_fee_declined is null').where('orders.fee > 0').distinct }
   ...
   def other_unpaid_invoices?(exception_order_id)
      q = "invoices.date_fee_paid is null and invoices.date_fee_declined is null and orders.order_status<>?"
      q << " and orders.fee > 0 and orders.id != #{exception_order_id.to_i}"
      return invoices.where(q,"canceled").count > 0
   end
