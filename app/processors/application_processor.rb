...
# Written: January - March 2010
  ...
  def on_error(err)
    if err.is_a? StandardError
      ...
      msg = ActiveSupport::JSON.encode({ :active_error => true, :messagable => @bmessagable, :pid => @pid, :message_type => 'error', :app => 'tracksys', :processor => self.class.name.demodulize, :workflow_type => @workflow_type, :message => err.message, :class_name => err.class.name, :backtrace => err.backtrace.join("\n")})
  ...
  def on_success(message)
    msg = ActiveSupport::JSON.encode({ :messagable => @bmessagable, :pid => @pid, :message_type => 'success', :app => 'tracksys', :processor => self.class.name.demodulize, :workflow_type => @workflow_type, :message => message})
    publish :automation_message, msg
  ...
  def on_failure(message)
    msg = ActiveSupport::JSON.encode({ :messagable => @bmessagable, :pid => @pid, :message_type => 'failure', :app => 'tracksys', :processor => self.class.name.demodulize, :workflow_type => @workflow_type, :message => message})
    publish :automation_message, msg
