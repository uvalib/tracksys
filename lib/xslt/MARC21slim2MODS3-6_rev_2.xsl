...
<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim"
  ...
  <xsl:variable name="progVersion">1.119.33</xsl:variable>
  ...
  1.119.07 - Modify datafield 655 subfield delimiter.
...
-->
    ...
    <xsl:if test="$typeOf008 = 'BK'">
      ...
      <xsl:choose>
        ...
        <xsl:when test="contains($controlField008-28, 'u')">
          <genre authority="marcgt">Unknown if item is government publication</genre>
        </xsl:when>
    ...
    <originInfo>
      ...
      <xsl:if test="$controlField008-6 = 't'">
        <xsl:if test="$controlField008-11-14">
          <!-- UVA Revision: 1.119.22 -->
          <copyrightDate encoding="edtf" keyDate="yes">
